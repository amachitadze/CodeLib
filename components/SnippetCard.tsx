...
import { Copy, Trash2, Eye, Code as CodeIcon, Maximize2, X, Save, FilePlus, Search, ArrowDown, Columns, Monitor, Heart, Tag, Sun, Moon, Info, ExternalLink, Download, Image as ImageIcon } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-sm flex flex-col animate-in fade-in duration-200" role="dialog" aria-modal="true">
      <div className="flex justify-between items-center px-4 sm:px-6 py-3 border-b border-slate-700 bg-slate-900 text-white shrink-0">
          ...
          <div className="flex items-center gap-2 ml-4 shrink-0">
            ...
            {!fsShowSaveAs && (
              <button onClick={() => setFsShowSaveAs(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg font-medium text-sm bg-slate-800 text-slate-400 hover:text-green-400 transition-all" title={t.card_save_as}>
             ...
             <div className="flex-1 flex flex-col md:flex-row bg-white overflow-y-auto">
                <div className="md:w-2/3 p-8 border-r border-slate-200">
                   {snippet.imageUrl ? (
                     ...
                     <div className="w-full aspect-video bg-slate-100 flex items-center justify-center rounded-xl mb-8">
      ...
      <div className={`bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col ${isTemplate ? 'h-auto pb-4' : 'h-[500px]'} transition-all duration-300 hover:shadow-md relative group`}>
        ...
        {isTemplate ? (
          <div className="p-4 flex flex-col gap-4">
             <div className="aspect-video w-full rounded-lg overflow-hidden border border-slate-100 relative group/img bg-slate-50">
                {snippet.imageUrl ? (
             ...
             <div className="grid grid-cols-2 gap-3">
                <a href={snippet.demoUrl || '#'} target={snippet.demoUrl ? "_blank" : "_self"} rel="noopener noreferrer" className={`flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-xs transition-all border ${snippet.demoUrl ? 'bg-indigo-50 text-indigo-700 border-indigo-100 hover:bg-indigo-100' : 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed'}`}><ExternalLink size={14}/> {t.card_demo_link}</a>
                <a href={snippet.downloadUrl || '#'} target={snippet.downloadUrl ? "_blank" : "_self"} rel="noopener noreferrer" className={`flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-xs transition-all ${snippet.downloadUrl ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}><Download size={14}/> {t.card_download_link}</a>
             </div>
          </div>
        ) : (
          <>
