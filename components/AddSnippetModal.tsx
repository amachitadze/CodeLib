...
const DEFAULT_CATEGORIES = {
  ...
  template: {
    ka: ['პორტფოლიო', 'ბიზნესი', 'SaaS', 'პერსონალური', 'E-commerce', 'სხვა'],
...
export const AddSnippetModal: React.FC<AddSnippetModalProps> = ({ isOpen, onClose, onSave, existingCategories, lang }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (type === 'template') return; // AI doesn't generate "template" objects directly here
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              >
                <ImageIcon size={24} />
                <span className="text-sm font-medium">{t.add_type_template}</span>
              </button>
            </div>
              ...
              <div className="space-y-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {imageUrl && (
                      <div className="w-12 h-12 rounded border border-slate-200 overflow-hidden relative group">
                        <img src={imageUrl} alt="Preview" className="w-full h-full object-cover" />
                        <button onClick={() => setImageUrl('')} className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white"><X size={14}/></button>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="lg:w-1/3 flex flex-col gap-4">
            <div className="bg-indigo-50/50 rounded-xl p-5 border border-indigo-100 flex flex-col h-fit">
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-indigo-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm min-h-[100px] resize-none mb-4 disabled:opacity-50"
                />
                ...
                <button
                  ...
                  disabled={isGenerating || !aiPrompt || type === 'template'}
                  type="button"
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
          ...
          <button
            ...
            type="button"
            disabled={!title || isUploading || (type !== 'template' && !code)}
